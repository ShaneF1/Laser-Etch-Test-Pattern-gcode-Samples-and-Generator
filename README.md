...
Below are examples of painted test tiles etched using these patterns. The first is a red tile painted first with yellow then top coated in white. The second is a yellow tile paint first in blue then top coated in brown. You could just as easily be using the pattens to determine etch depth in wood, acrylic or glass.
